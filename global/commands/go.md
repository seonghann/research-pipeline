...
- **fix** → return to Phase 2 with specific issues to address

### Phase 4: Post-report & Commit
1. Update the progress log with post-report (sections 3-6):
   - Section 3.1: Iteration History (link to analyze scripts created during cycle)
...
- Integrity gates are enforced in Phase 2 AND Phase 3 (defense in depth).
- All investigate/debug artifacts go in `analyze/` (permanent record).
