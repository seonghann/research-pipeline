...
description: "Run the agreed full research cycle with analysis, implementation, verification and checkpoints."
---
...
- [prereport](references/agents/prereport.md)
- [analyzer](references/agents/analyzer.md)
- [implementer](references/agents/implementer.md)
...
Codex have a shared understanding of the problem. If not, have the conversation
...
2. Delegate to `analyzer` agent (**include context**: why this question,
   ...
   ```
   Delegation brief("Analyze: <question>. Context: <why, related files, previous findings>")
   ```
...
```
Delegation brief("Implement: <progress log path>. 분석 결론: <key findings>. 합의 사항: <decisions>.")
```
...
- **n** → stop and preserve the working tree; discuss any scoped undo
...
2. Stage and commit:
   ```
   git add -- <explicit-task-owned-file-paths>
