...
Called via: `Delegation brief("Generate pre-report for: <task description>")`
...
1. **Read project AGENTS.md** for project-specific context and current state.
...
6. **Delegate for external context** (if relevant to the task):
   - Need literature background? → `Delegation brief("Papers: <topic>")`  (paper-fetcher)
   - Need experiment results? → `Delegation brief("Experiments: <query>")` (experiment-fetcher)
   - Need reference implementation? → `Delegation brief("Explore repo: <url> — <question>")` (repo-explorer)
...
- Native subagent tools (delegate to paper-fetcher, experiment-fetcher, repo-explorer)
